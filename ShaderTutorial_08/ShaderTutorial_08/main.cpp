...
D3DXMATRIX				g_viewMatrixRotInv;	//カメラの回転行列の逆行列。
...
void InitProjectionMatrix()
{
	...
	D3DXMATRIX viewInv;
	g_viewMatrixRotInv = g_viewMatrix;
	//平行移動成分を打ち消す。
	g_viewMatrixRotInv.m[0][3] = 0.0f;
	g_viewMatrixRotInv.m[1][3] = 0.0f;
	g_viewMatrixRotInv.m[2][3] = 0.0f;
...
VOID Render()
{
	...
	if (SUCCEEDED(g_pd3dDevice->BeginScene()))
	{
		...
		g_pEffect->SetMatrix("g_viewMatrixRotInv", &g_viewMatrixRotInv);
		//回転行列を転送。
