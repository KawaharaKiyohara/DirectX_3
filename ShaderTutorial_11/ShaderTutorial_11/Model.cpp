...
void CModel::Draw(D3DXMATRIX* viewMatrix, D3DXMATRIX* projMatrix, bool isDrawShadowMap, bool isRecieveShadow)
{
	...
	//シャドウレシーバーフラグを転送。
	g_pModelEffect->SetInt("g_isShadowReciever", isRecieveShadow);
	//ワールド行列の転送。
	...
	// a loop
	if (isRecieveShadow) {
		g_pModelEffect->SetTexture("g_shadowMapTexture", g_shadowMap.GetTexture());
		g_pModelEffect->SetMatrix("g_lightViewMatrix", &g_shadowMap.GetLightViewMatrix());
		g_pModelEffect->SetMatrix("g_lightProjectionMatrix", &g_shadowMap.GetLightProjectionMatrix());
	}
	for (DWORD i = 0; i < dwNumMaterials; i++)
	{
