...
float					g_modelAlpha = 1.0f;		//モデルのα。
float					g_alphaDelta = -0.02f;		//αの変位量。
/*!
...
VOID Render()
{
	...
	{
		g_modelAlpha += g_alphaDelta;
		if (g_modelAlpha <= 0.0f) {
			g_modelAlpha = 0.0f;
			g_alphaDelta *= -1.0f;
		}
		else if (g_modelAlpha >= 1.0f) {
			g_modelAlpha = 1.0f;
			g_alphaDelta *= -1.0f;
		}
		// Turn on the zbuffer
		...
		g_pEffect->SetFloat("g_modelAlpha", g_modelAlpha);
