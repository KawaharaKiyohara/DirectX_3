...
VOID Render()
{
	...
	if (SUCCEEDED(g_pd3dDevice->BeginScene()))
	{
		...
		//CPUで計算を行う。
		D3DXMatrixMultiply(&g_wvpMatrix, &g_worldMatrix, &g_viewMatrix);
		D3DXMatrixMultiply(&g_wvpMatrix, &g_wvpMatrix, &g_projectionMatrix);
		//ワールドビュープロジェクション行列の転送。
		g_pEffect->SetMatrix("g_wvpMatrix", &g_wvpMatrix);
